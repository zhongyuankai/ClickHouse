...
engine = ReplicatedMergeTree('/test/03005/{database}/rmt', '1')
order by (k, d)
partition by toYYYYMMDD(d);

insert into forget_partition select number, toDate('2024-01-01') + interval number day, randomString(20) from system.numbers limit 10;
...
select '---before---';
select name from system.zookeeper where path = '/test/03005/' || currentDatabase() || '/rmt/block_numbers' order by name;
