...
create table td2 engine = Distributed(test_shard_localhost, currentDatabase(), 't', xxHash32(val), 1, default) as t;
create table td3 engine = Distributed(test_shard_localhost, currentDatabase(), 't', xxHash32(val), 0, 'default') as t;
drop table if exists t;
drop table if exists td1; 
drop table if exists td2; 
drop table if exists td3;
