...
SELECT count(*) FROM tab PREWHERE b = 10000 SETTINGS use_query_condition_cache = true;
...
WHERE
    ...
    AND query = 'SELECT count(*) FROM tab PREWHERE b = 10000 SETTINGS use_query_condition_cache = true;'
ORDER BY
    event_time_microseconds;

SELECT * FROM tab PREWHERE b = 10000 SETTINGS use_query_condition_cache = true;
...
WHERE
    type = 'QueryFinish'
    AND current_database = currentDatabase()
    AND query = 'SELECT * FROM tab PREWHERE b = 10000 SETTINGS use_query_condition_cache = true;'
ORDER BY
    event_time_microseconds;
