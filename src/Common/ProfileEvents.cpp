...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(QueryConditionCacheHits, "Number of times an entry has been found in the query condition cache (and reading of marks can be skipped). Only updated for SELECT queries with SETTING use_query_condition_cache = 1.") \
    ...
    M(DistributedConnectionNotLeader, "Number of connections that are not leader nodes.") \
...
The server successfully detected this situation and will download merged part from replica to force byte-identical result.
    ...
    \
    M(PartitionUniquerWriteLockMicroseconds, "Partition uniquer write lock time.") \
    M(PartitionUniquerMergeLockMicroseconds, "Partition uniquer merge lock time.") \
    \
