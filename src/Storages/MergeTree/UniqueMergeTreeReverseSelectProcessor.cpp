...
UniqueMergeTreeReverseSelectProcessor::UniqueMergeTreeReverseSelectProcessor(
    ...
    bool quiet):
        MergeTreeReverseSelectProcessor(storage_, metadata_snapshot_, owned_data_part_, max_block_size_rows_,
            preferred_block_size_bytes_, preferred_max_column_in_block_size_bytes_, required_columns_, mark_ranges_,
            use_uncompressed_cache_, prewhere_info_,
...
void UniqueMergeTreeReverseSelectProcessor::initializeRangeReaders(MergeTreeReadTask & current_task)
{
    if (prewhere_info)
    {
