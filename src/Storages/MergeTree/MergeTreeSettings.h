...
#define LIST_OF_MERGE_TREE_SETTINGS(M, ALIAS) \
    ...
    M(Bool, ignore_merge_type, false, "Compatible with lower version replica, the merge type cannot be recognized.", 0) \
    ...
    M(Bool, skip_broken_part, false, "For MergeTree, skip the broken parts when reading, no exception will be thrown, such as disk broken and data corruption.", 0) \
    \
    ...
    \
    M(String, default_compression_codec, "", "Specifies the default compression codec to be used if none is defined for a particular column in the table declaration. Default value: an empty string (not defined).", 0) \
