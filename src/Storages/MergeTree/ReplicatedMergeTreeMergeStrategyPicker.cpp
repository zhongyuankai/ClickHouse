...
{
    /// For ReplicatedUniqueMergeTree, only one replica is allowed to merge. The reason is that column `_unique_id` is unstable.
    /// The checksums of the Parts generated by merging different copies are different.
...
{
    /// For UniqueMergeTree, only the leader copy is allowed to perform merge.
    if (storage.merging_params.mode == MergeTreeData::MergingParams::Unique && storage.getSettings()->execute_merges_on_leader_replica)
    {
        if (storage.isLeader())
            return std::nullopt;

        if (storage.leader_election)
        {
            return storage.leader_election->leaderReplicaId();
        }
        return "";
    }
