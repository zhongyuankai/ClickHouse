...
bool ReplicatedMergeTreeRestartingThread::runImpl()
{
    ...
    if (storage.snapshot_updating_task)
        storage.snapshot_updating_task->activateAndSchedule();
...
bool ReplicatedMergeTreeRestartingThread::tryStartup()
{
    ...
    try
    {
        ...
        {
            /// Table was created before 20.4 and was never alterd,
...
Int32 ReplicatedMergeTreeRestartingThread::fixReplicaMetadataVersionIfNeeded(zkutil::ZooKeeperPtr zookeeper)
{
    ...
    for (size_t attempt = 0; attempt < num_attempts; ++attempt)
    {
        ...
        {
            LOG_INFO(log, "Skipping updating metadata_version as there ar ALTER_METADATA entries in the queue");
            return metadata_version;
        }
    ...
    /// Second attempt is only possible id either metadata_version.version or metadata.version changed during the first attempt.
